...
const PurchasePage: React.FC<PurchasePageProps> = ({ params }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const dataMovieDetails = await getDetailsMovie({ id });
    ...
    return (
        <div className="h-full flex flex-col items-center bg-gray-100 text-black">
            <main className="w-full flex flex-col items-center p-6">
                ...
                <div className="w-full max-w-xl mt-4 bg-white text-black p-6 rounded-lg shadow-md">
                    <h2 className="text-xl font-bold mb-2">Select Date:</h2>
                    <input 
                        type="date" 
                        value={showDate}
                        onChange={(e) => setShowDate(e.target.value)}
                        className="border p-2 w-full rounded-md my-2 text-black"
                    />
                    <h2 className="text-xl font-bold mb-2 text-black">Select Time:</h2>
                    <div className="grid grid-cols-4 gap-4 my-2">
                        {showTimes.map((time) => (
                            <button
                                key={time}
                                onClick={() => handleTimeSelection(time)}
                                className={`p-2 ${selectedTime === time ? 'bg-green-500 text-white' : 'bg-gray-200 text-black'} rounded-md hover:bg-green-400 transition-colors`}
                            >
                                {time}
                            </button>
                        ))}
                    </div>
                    <h2 className="text-xl font-bold mb-2 text-black">Select Seats:</h2>
                    <div className="grid grid-cols-8 gap-4 my-2">
                        {availableSeats.map((seat) => (
                            <button
                                key={seat}
                                onClick={() => handleSeatSelection(seat)}
                                className={`p-2 ${selectedSeats.includes(seat) ? 'bg-green-500 text-white' : 'bg-gray-200 text-black'} rounded-md hover:bg-green-400 transition-colors`}
                            >
                                {seat}
                            </button>
                        ))}
                        ...
                        {selectedSeats.map((seat) => (
                            ...
                            >
                                ...
                                <div className="flex flex-col items-center">
                                    ...
                                    <p className="text-xs text-gray-500 mt-2">CinemaApp</p>
                                </div>
                            </div>
                        ))}
                    </div>
